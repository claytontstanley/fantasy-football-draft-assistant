...
# I couldn't get historical data for defenses that had every piece of information needed to regenerate scores.
# So instead of the typical approach, defense scores are generated by looking at ESPN's predicted DF scores for 
# the upcoming season, and then recalibrating those scores based on how much their scores for kickers deviate from 
...
generateDfs = function() {
	...
	data.frame(name=dfFrm$PLAYER..TEAM.POS, rank=dfFrm$RNK, displayType='df', score=with(dfFrm, mA+PTS*mB))
}
